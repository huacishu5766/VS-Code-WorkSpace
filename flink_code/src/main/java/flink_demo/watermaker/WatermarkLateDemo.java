 ...
 * 2、乱序、迟到教据的处理
 *  1)watermark中指定乱宁等待时间
 *  2)如果开窗，设置窗口允许迟到
 *          =》 推迟关窗时间，在关窗之前，迟到数据来了，还能被窗口计算，来一条迟到数据触发一次计算
 *          =》 关窗后，迟到数据不会被计算
 *  3)关窗后的迟到数据，放入侧输出流
 *
 *  如果 watermark等待3s，窗口允许迟到2s，为什么不直接 watermark等待5s或者窗口允许迟到5s?
 *  ===》 watermark等待时间不会设太大    ===》影响的计算延迟
 *          如果3s ==》 窗口第一次触发计算和输出，13s的数据来 13 - 3 = 10s
 *          如果5s ==》 窗口第一次触发计算和输出，15s的数据来 15 - 5 = 10s
 * ===》 窗口允许迟到，是对大部分迟到数据的处理，尽量让结果准确如果只设置 允许迟到5s，那么就会导致 频繁 重新输出
 * TODO 设置经验
 * 1、watermark等待时间，设置一个不算特别大的，一般是秒级，在乱序和延迟 取舍
 * 2、设置一定的窗口允许迟到，只考虑大部分的迟到数据，极端小部分迟到很久的数据不管
 * 3、极端小部分迟到很久的数据，放到侧输出流，获取到之后做各种处理
 */